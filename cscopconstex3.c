...
char fnamer[122]="/Users/khomma/work/ensembl/cele/cscoplist12";
char fnamel[100]="/Users/khomma/work/ensembl/cele/calluniqexon3M";      /* Non-redundant list of exons, with int CDS length */
char fnamew[122]="/Users/khomma/work/ensembl/cele/cscopconstex3";       /* #+# */
	...
	for(z=0; z<10; z++){	/* 0:All ex len, 1:1~120, 2:~240, 3:~360, 4:~480, 5:~600, 6:~720, 7:~840, 8:~960, 9:961~ bp */
		...
		if(rstr[0]=='>'){
			...
			if(counter%1000==0)	printf("Now processing variant %d\n",counter);
                        ...
                        /* ### DEBUGGED to convert WBGene9977_388 into  WBGene00009977, for example */
                        strncpy(tempstr,empty,21);
                        for(x=0; x<50; x++)     if(isdigit(rstr[x])!=0)         break;
                        offset = x;

                        /* Get 9977 from >WBGene9977_388, for example */
                        for(x=0; x<20; x++){
                                if(isdigit(rstr[x+offset])==0)  break;
                                tempstr[x] = rstr[x+offset];
                        }
                        w = strlen(tempstr);
                        strncpy(idstr,empty,51);        strcpy(idstr,"WBGene");
                        if(w<=7){
                                for(x=0; x<=7-w; x++)  strcat(idstr,"0");       /* ### DEBUGGED ### */
                        }
			...
			strncpy(lstr,empty,201);
				...
				if(strncmp(lstr,"MtDNA",5)==0)  continue;       /* #### */
