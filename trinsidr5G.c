...
char fnamer[122]="/Users/khomma/work/dichot/ndichot1";          /* DICHOT results */
char fnamec[122]="/Users/khomma/work/ensembl/rat/tptcorr1";     /* Correspondence between protein(ENSRNOP) and transcr.(ENSRNOT) IDs */
char fnamee[122]="/Users/khomma/work/ensembl/rat/nallexon3";   	/* Revised exon list */
char fnamei[122]="/Users/khomma/work/ensembl/hmr/trinsexon1G";    /* NEW list of inserted segments */
char fnamew[122]="/Users/khomma/work/ensembl/hmr/trinsidr5G";    /* #&# */
char instr[10000][51];
...
int main(void)
{
	...
	strncpy(istr,empty,501);
		...
		if(strstr(istr,"ENSRNOT")!=NULL)        insread();
		strncpy(istr,empty,501);
	}
	fclose(fi);
	...
	strncpy(protid,empty,51);	strncpy(trid,empty,51);		plen = -1; 	imax = -1;	hitflag = 0;
		...
		if(rstr[0]=='>'){
			++counter;
			if(counter%2000==0)	printf("Now processing entry %d out of 16,405...\n",counter);
			hitflag = 1;
			offset = 1;
			for(x=0; x<50; x++){
		...
		else if(hitflag==4 && (strncmp(rstr,"KNWDOM",6)==0 || strncmp(rstr,"NEWDOM",6)==0)){       /* SD line */
			sdread();
		}
