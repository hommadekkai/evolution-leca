...
void ecread(char rstr[1001]){	/* Parse the exon line to get the sense, start, end, and chromosome # */
	...
	offset = x + offset;
	for(x=0; x<50; x++)	if(isspace(rstr[x+offset])!=0) break;		/* The end of WormBase */
	offset = x + offset;
	for(x=0; x<50; x++)	if(isspace(rstr[x+offset])==0)	break;
	offset = x + offset;
...
{
	char rstr[1001];
        char fnamer[122]="/Users/khomma/work/ensembl/cele/Caenorhabditis_elegans.WBcel235.97.gtf";
        char fnamew[122]="/Users/khomma/work/ensembl/cele/callexon3M";		/* #"# */
	...
	for(x=0; x<40000; x++){
        ...
        while((fgets(rstr,1000,fr))!=NULL){
                if(rstr[0]!='#' && strstr(rstr,"WormBase")!=NULL && strstr(rstr,"gene")!=NULL && strstr(rstr,"gene_id")!=NULL){
                        /* Verify that the new gene has actually been shown */
                        ps = strstr(rstr,"WormBase");
                        pt = strstr(rstr,"gene");
                        /*      printf("pt-ps = %ld in %s",pt-ps, rstr);                */
                        if(pt-ps>9){    /* Was a false hit */
                                if(strstr(rstr,"transcript")!=NULL)     goto transcr;
				...
				calcintcds();   /* #"# Calculate the legth of the CDS encoded by internal exons */
                                if(exmax>=0){
                        ...
                        /* Read in the gene name (e.g., WBGene00198386") */
                        strncpy(gstr,empty,1001);
                        ps = strstr(rstr,"gene_id");
                        ps += 9;
                        for(x=0; x<1000; x++){
		...
		else if(strstr(rstr,"WormBase")!=NULL && strstr(rstr,"transcript")!=NULL && strstr(rstr,"protein_coding")!=NULL){
transcr:                /* Possibly a new mRNA, i.e., a new set of exons and CDS must be initiated */
                        /* Verify */
                        ps  = strstr(rstr,"WormBase");
                        pt = strstr(rstr,"transcript");
                        if(pt-ps>9){
				...
				calcintcds();   /* #"# Calculate the legth of the CDS encoded by internal exons */
                        ...
                        /*  Read in the transcript name (e.g., "B0348.6c.1") */
                        if(strstr(rstr,"transcript_id")!=NULL){
				strncpy(trname,empty,51);	/* ##&& Debugged (220222) */
                                ps = strstr(rstr,"transcript_id");
                                ps += 15;
                                for(x=0; x<50; x++){
                                        if(*ps=='"' || isprint(*ps)==0)     break;
                                        trname[x] = *ps;
