...
char fnamer[122]="/Users/khomma/work/ensembl/human/hscoplist12";
char fnamel[100]="/Users/khomma/work/ensembl/human/halluniqexon3M";	/* Non-redundant list of exons, with int CDS length */
char fnamew[122]="/Users/khomma/work/ensembl/human/hscopconstex3";	/* #++# */
	...
	for(z=0; z<10; z++){	/* 0:All ex len, 1:1~120, 2:~240, 3:~360, 4:~480, 5:~600, 6:~720, 7:~840, 8:~960, 9:961~ bp */
		...
		if(rstr[0]=='>'){
			...
			if(counter%1000==0)	printf("Now processing variant %d\n",counter);
                        ...
                        /* DEBUGGED to convert ENSG6837_401 into ENSG00000006837, for example */
                        strncpy(tempstr,empty,21);
                        for(x=0; x<50; x++)     if(isdigit(rstr[x])!=0)         break;
                        offset = x;

                        /* Get 6837 from >ENSG6837_401, for example */
                        for(x=0; x<20; x++){
                                if(isdigit(rstr[x+offset])==0)  break;
                                tempstr[x] = rstr[x+offset];
                        }
                        w = strlen(tempstr);
                        strncpy(idstr,empty,51);        strcpy(idstr,"ENSG");
                        offset = 5;
                        for(x=0; x<=10-w; x++)  strcat(idstr,"0");
			...
			strncpy(lstr,empty,201);
				...
				if(strncmp(lstr,"MT",2)==0)	continue;	/* #### */
				else if(strstr(lstr,idstr)!=NULL){	/* Possible hit. Check the total length */
					...
					else if(length!=w && hitflag==1)	break;	/* The end of the transcript */
				}
