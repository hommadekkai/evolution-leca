...
void ecread(char rstr[1001]){	/* Parse the exon line to get the sense, start, end, and chromosome # */
	...
	offset = x + offset;
	for(x=0; x<50; x++)	if(isspace(rstr[x+offset])!=0) break;		/* The end of RAP2018-11-26 */
	offset = x + offset;
	for(x=0; x<50; x++)	if(isspace(rstr[x+offset])==0)	break;
	offset = x + offset;
...
void exonrange( void ){         /* &&&  Determine the range of a.a. residues each exon encodes, if any */
        ...
        if(cds[0][0]>0){        /* Positive strand */
                ...
                for(y=cdsmax; y>=0; y--){
			cumres += cds[y][2] - cds[y][1] + 1;    /* ### This line was missing ###  The end nucleotide number of the CDS */
                        w = (cumres/3) + 1;     /* The end res. # of the current exon */
...
{
	char rstr[1001];
	char fnamer[122]="/Users/khomma/work/ensembl/rice/Oryza_sativa.IRGSP-1.0.44.chr.gff3";
        char fnamew[122]="/Users/khomma/work/ensembl/rice/rallexon3M";		/* #"# */	
	...
	for(x=0; x<40000; x++){
		...
		if(strncmp(rstr,"###",3)==0){
		...
		else if(strstr(rstr,"RAP2018-11-26")!=NULL && strstr(rstr,"ID=gene:")!=NULL){
                        if(gflag==1 && exmax>=0 && cdsmax>=0){  /* Output the last variant */
                                ++hit;
                                if(exmax>emax)  emax = exmax;
				calclen();      /* Calculate the length of the CDS */
				...
				for(y=0; y<4; y++)	cds[x][y]=0;		/* &&& */
                        ...
                        /* Read in the gene name (e.g., "Os01g0100100") */
                        strncpy(gstr,empty,1001);
		...
		else if(gflag==1 && strstr(rstr,"RAP2018-11-26")!=NULL && strstr(rstr,"mRNA")!=NULL){
             		/* Possibly a new mRNA, i.e., a new set of exons and CDS must be initiated */
                        /* Verify */
                        ps = strstr(rstr,"RAP2018-11-26");
                        pt = strstr(rstr,"mRNA");
                        if(pt-ps!=14){
				...
				calcintcds();   /* #### Calculate the range of the CDS encoded by internal exons */
                        ...
                        /* Read in the transcript name (e.g., "Os01t0100100-01") */
		...
		else if(gflag==1 && strstr(rstr,"RAP2018-11-26")!=NULL && (strstr(rstr,"exon")!=NULL || strstr(rstr,"CDS")!=NULL)){
ecd:                    /* Process the exon or CDS line */
                        /* Verify the hit first: */
                        ps = strstr(rstr,"RAP2018-11-26");
                        if(strstr(rstr,"exon")!=NULL)   pt = strstr(rstr,"exon");
                        else                            pt = strstr(rstr,"CDS");
                        if(pt-ps!=14)   goto nextr;     /* Was a false hit */
                        ecread(rstr);
                }
