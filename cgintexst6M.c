...
#include <math.h>
FILE *fr, *fi, *fw, *fl;
char fnamei[122]="/Users/khomma/work/disopred/cdisopr12";               /* DISOPRED3 results including variant data */
char fnamer[122]="/Users/khomma/work/ensembl/cele/calluniqexon3A";      /* ### Corrected list of ALL internal exons */
char fnamew[122]="/Users/khomma/work/ensembl/cele/cgintexst6M";          /* ### Avg and median values */
char fnamel[122]="/Users/khomma/work/ensembl/cele/cgintexstlist6M";      /* ### List of data */
char binstr[10][20]={"All","-120","-240","-360","-480","-600","-720","-840","-960","961-nt"};	/* #### */
char rstr[5001], istr[5001], empty[5010]="", tempstr[21], entryname[51], abbentname[51], outstr[500], listread[21];
char tchr[10], tgname[51], oldgname[51], ttname[51], oldtname[51], tsense[2];
int  p, q, t, u, v, w, x, y, z, offset, excount=0, vcounter=0, vhit=0, hitcount=0;
...
void assign(void)	/* Find the SD assignments, if any */
{
	...
	strncpy(istr,empty,5001);
		...
		if(istr[0]=='>' && strstr(istr,entryname)!=NULL){	/* Possible hit */
		...
		else if(istr[0]=='>' && strstr(istr,abbentname)!=NULL){/* Possible hit with the alternative name */
			ihit = 1;
		}
			...
			aw = atoi(tempstr);
			if(aw-1!=oldcdslen)     ihit = 0;       /* %%% False hit (a different variant) */
...
int main(void)
{
	...
	strncpy(entryname,empty,51);		strncpy(abbentname,empty,51);
        vmax = -1;
	strncpy(rstr,empty,5001);
        while((fgets(rstr,5000,fr))!=NULL){
		if(rstr[0]=='>')	goto nextu;
		else if(strncmp(rstr,"MtDNA",5)==0)        goto nextu;               /* ### Exclude non-nuclear coded genes */
		else{
			++excount;
			if(excount%5000==0)	printf("Now processing exon %d out of 87,985...\n",excount);	 /* #### */
			...
			if(strcmp(ttname,oldtname)==0){	
				...
				/* Find the IDR and SD assignments: */
				strncpy(entryname,empty,51);		strncpy(abbentname,empty,51);
				strcpy(entryname,oldgname);
                                strcpy(abbentname,"WBGene");
                                for(x=0; x<40; x++){            /* e.g., WBGene0002099-> abbentname:WBGene2099 */
                                        if(isprint(entryname[x])==0)    break;
                                        abbentname[x+6] = entryname[x+9];
                                }
