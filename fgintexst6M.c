...
#include <math.h>
FILE *fr, *fi, *fw, *fl;
char fnamei[122]="/Users/khomma/work/disopred/fdisopr12";               /* DISOPRED3 results including variant data */
char fnamer[122]="/Users/khomma/work/ensembl/spomb/falluniqexon3A";     /* ### Corrected list of ALL internal exons */
char fnamew[122]="/Users/khomma/work/ensembl/spomb/fgintexst6M";        /* ### Avg and median valued */
char fnamel[122]="/Users/khomma/work/ensembl/spomb/fgintexstlist6M";    /* ### List of data */
char binstr[10][20]={"All","-120","-240","-360","-480","-600","-720","-840","-960","961-nt"};	/* #### */
char rstr[5001], istr[5001], empty[5010]="", tempstr[21], entryname[51], convname[51], outstr[500], listread[21];
...
void assign(void)	/* Find the SD assignments, if any */
{
	...
	strncpy(istr,empty,5001);
		...
		if(istr[0]=='>' && strstr(istr,convname)!=NULL){/* Possible hit with the alternative name */
			ihit = 1;
		}
...
int main(void)
{
	...
	strncpy(entryname,empty,51);		strncpy(convname,empty,51);
        vmax = -1;
	strncpy(rstr,empty,5001);
        while((fgets(rstr,5000,fr))!=NULL){
		if(rstr[0]=='>')	goto nextu;
		else if(strncmp(rstr,"MT",2)==0)        goto nextu;               /* Exclude non-nuclear coded genes */
		else if(strncmp(rstr,"AB325691",8)==0)	goto nextu;	
		else{
			++excount;
			if(excount%5000==0)	printf("Now processing exon %d out of 187,484...\n",excount);	 /* #### */
			...
			if(strcmp(ttname,oldtname)==0){	
				...
				strcpy(entryname,oldgname);
                                for(x=0; x<40; x++){            /* e.g., SPAC1639.01c -> convname:SPAC1639_01c */
                                        if(isprint(entryname[x])==0)    break;
                                        else if(entryname[x]=='.')      convname[x] = '_';
                                        else    convname[x] = entryname[x];
                                }
