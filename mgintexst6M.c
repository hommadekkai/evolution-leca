/* mgintexst6M.c  This program determines the median # of residues in SD in each exon length range of all genuine internal exons  220206
	basic.c -> hnmdcnumb1.c: A new program [R1906 I].		  
	 -> hnmdidr1.c: Revision to calculate per cent IDR by POODLE-L prediction [R1906 II].
	 -> hnmdmsidr1.c: Revision to divide proteins into those encoded by single- and multiple- exons [R1906 IV].
	 -> hnmdmsidr1R.c: Revision to make use of revised data with notes on NMD-degrated genes [R1906 IV].
	 -> hnmdSidr4.c: Revision to process single-exon vs NMD-functional using new POODLE results [R1913 I].
	 -> hnmdSidr4B.c: Disregard the cnumbinction between single and multi exons and concentrate on NMD-functions [R1913 I].
	 -> anmdidr4.c: Revised to process A. thaliana instead of human data [R1913 II].
	 -> cnmdidr4.c: Revised to process C. elegans instead of A. thaliana data [R1913 III].
	 -> fnmdidr4.c: Revised to process S. pombe instead of C. elegans data [R1913 IV].
	 -> fnmdidr4N.c: Revised to proess new S. pombe data [R2001 IV].
	 -> hnmdidr4N.c: Revised to process human instead of S. pombe data [R2001 I].
	 -> hnmdidr4T.c: Revised to output the harmonic mean of #NMD and #Non-NMD at each x [R2001 I].
	 -> hlexidr4.c: Revised to classify according to exons instead of NMD [R2003 I].
	 -> hcexidr4.c: Revised to single out the C-terminal 5 exons instead of just one [R2003 III].
	 -> hcexidr4L.c: Revised to increase the # of C-term. exons to 10 and to increase the max. # of res. to 5000 [R2003 III].
			Also, modified so that cnumb. from the terminus (x) larger than 5000 is registered at x=4999.
	 -> hrexidr4.c: Revised to use a new method for calculating delta per cent IDR[x] [R2003 IV].
	 -> hexterm4.c: Revised to determine the %IDR dep. on the cnumb. from the term. of each exon 
			AND to use SDs instead of IDRs for calculation [R2005 I].
	 -> hexolen4.c: Major revision to calculate the dep. of %IDR on exon length [R2007 I].
	 -> hrexon4.c:	Modified to calculate the correlations of %IDR and exon length of each exon from both termini [R2007 I].
	 -> hrdelx4.c: Major revision to find the dep. of %SD instead of %IDR on delta x (the dist. from the 5' end of each exon) [R2007 I].
	 -> hrdelx4M.c: Limit analysis to the 5' half of each exon (#!#: modified lines) [R2007 I].
	 -> hexdelx4.c: Divide exons into length bins (u=1~7) and find the correlations [R2007 I].
	 -> hexdelz4.c: Modified not to stop at the middle of each exon and to calculte that total of N- and C-terminal exons [R2007 I].
		Also modified to include all bins with non-zero res. # instead of those with 10 or more. Moreover, calculate and
		output statistics of SD contents as hexdelz4C. Furthermore, determine the corr. between exon len. and fr. SD (#-# lines).
	 -> hexmed4.c: Greatly modified so that the median residue # of each exon length range can be determined [R2008 I].
	 -> hexmed4A.c: Modified to divide exon length bins in multiples of 80 instead of 40 a.a. [R2008 I].
	 -> hintexst4.c: Revised to read in the exon data of newly selected ALL internal exons [R2101 I].
	 -> hintexst4M.c: Revised to use a corrected exon list (###: mod.lines) [R2101 I].
			Also, determine and output the corr. of exon len. with %IDR (#&# lines).
	 -> hgintexst4M.c: Revised to process data of exons by the conventional definition [R2202 I].
			Also changed to 9 exon length bins in nt, instead of aa.
	 -> mgintexst4M.c: Modified to process mouse instead of human data (#####) [R2202 VII].
	 -> mgintexst6M.c: Revised to use DISOPORED3 instead of POODLE-L assignments [R2002 VII]. */
...
#include <math.h>
FILE *fr, *fi, *fw, *fl, *fc;		/* ##### */
char fnamei[122]="/Users/khomma/work/disopred/mdisopr1";                /* Partial DISOPRED3 results including variant data */
char fnamer[122]="/Users/khomma/work/ensembl/mouse/malluniqexon3A";	/* #### Corrected list of ALL internal exons */
char fnamew[122]="/Users/khomma/work/ensembl/mouse/mgintexst6M";	/* ### Avg and median values */
char fnamel[122]="/Users/khomma/work/ensembl/mouse/mgintexstlist6M";	/* ### List of data */
char fnamec[122]="/Users/khomma/work/ensembl/mouse/mptcorr1";           /* ##### ENSMUSP-ENSMUST correspondence */
...
char idcorr[70000][2][21];       /* ##### ENSMUSP-ENSMUST correspondence */
...
void assign(void)	/* Find the SD assignments, if any */
{
	...
	strncpy(istr,empty,5001);
		...
		else if(ihit==2 && strncmp(istr,"STRDOM",6)==0){	/* SD line */
			offset = 15;
			if(sdmax>=1000){
...
int main(void)
{
	...
	if((fi=fopen(fnamei,"r")) == NULL){     printf("Cannot open the input file %s\n", fnamei);      exit(1);        }
        ...
        for(x=0; x<70000; x++){
                for(y=0; y<2; y++)      /* 0: ENSMUSP, 1: ENSMUST */    strncpy(idcorr[x][y],empty,21);
        }
	...
	strncpy(rstr,empty,5001);
		...
		if(rstr[0]=='>')	goto nextu;
			...
			++excount;
			if(excount%10000==0)	printf("Now processing exon %d out of 178,114...\n",excount);	 /* #### */
			...
			if(strcmp(ttname,oldtname)==0){	
				...
				if(strlen(oldgname)<1)		goto update;	/* The first line, for which oldgname is empty */
                                ...
                                /* ##### Find the ENSP ID that corresponds to the ENST ID (oldtname) */
                                strncpy(entryname,empty,51);
                                for(x=0; x<=cmax; x++){
                                        if(strcmp(oldtname,idcorr[x][1])==0){   /* Found the correspondence */
                                                strcpy(entryname,idcorr[x][0]);         /* ENSP ID */
                                                goto okcorr;
                                        }
                                }
...
ctot:	/* Calculate the total of all exon length bins */
	...
	fputs("DISOPRED3\tLen. range\tNumber\tAvg len\tSEM\tAvg SD res.\t SEM\tAvg %SD\tSEM\n",fw);
