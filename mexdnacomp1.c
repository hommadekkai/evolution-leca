/* mexdnacomp1.c    This program examines the nucleotide freq in long internal exons compared to that in other exons		220223
	 basic.c -> hexdnacomp1.c: A new program [R2203 I].
	 -> mexdnacomp1.c: Modified to process mouse instead of human data [R2203 VII].   */
...
char fnamer[122]="/Users/khomma/work/ensembl/mouse/mabbcdnalist1";	/* Abbreviated cDNAs */
char fnamew[122]="/Users/khomma/work/ensembl/mouse/mexdnacomp1";
char fnamee[100]="/Users/khomma/work/ensembl/mouse/malluniqexon4AB";
...
void exsearch(void){	/* Look for the corresponding exon info. */
	...
	strncpy(estr,empty,501);	hitflag = 0;
		...
		if(strncmp(estr,"MT",2)==0)	goto nexte;	/* Neglect nonnuclear genes */	
		else if(estr[0]!='>' && strstr(estr,trid)!=NULL){	/* Possible hit */
			sread(estr);
...
int main(void)
{
	...
	for(x=0; x<200000; x++)		seq[x] = empty[0]; 
		...
		if(rstr[0]=='>'){
			...
			for(x=0; x<30; x++){
				if(rstr[x+offset]=='.' || isspace(rstr[x+offset])!=0)	break;
