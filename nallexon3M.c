...
char *ps, *pt;
char fnamer[122]="/Users/khomma/work/ensembl/rat/Rattus_norvegicus.Rnor_6.0.103.chr.gff3";
char fnamew[122]="/Users/khomma/work/ensembl/rat/nallexon3M";	/* #"# */
...
void ecread(void){	/* Parse the exon line to get the sense, start, end, and chromosome # */
	int excdsflag;
	strncpy(tempstr,empty,21);
	for(x=0; x<20; x++){
	...
	for(x=0; x<10000; x++){
        ...
        while((fgets(rstr,1000,fr))!=NULL){
			...
			tcount = 0;	/* The # of termnal exon regions per gene */
                ...
                else if(strstr(rstr,"ensembl")!=NULL || strstr(rstr,"havana")!=NULL){
			...
			for(x=0; x<20; x++)     if(isspace(rstr[x])!=0) break;          /* Chromosome # */
                        ...
                        for(x=0; x<50; x++)     if(isspace(rstr[x+offset])==0)  break;
                        offset = x + offset;
                        for(x=0; x<50; x++)     if(isspace(rstr[x+offset])!=0) break; /* The end of havana, ensemble, or ensemble_havana */
                        offset = x + offset;
                        for(x=0; x<50; x++)     if(isspace(rstr[x+offset])==0)  break;
                        offset = x + offset;
                        strncpy(class,empty,21);      /* Supposed to be mRNA */
                        for(x=0; x<20; x++){
                                if(isspace(rstr[x+offset])!=0)  break;
			...
			else if(strcmp(class,"exon")==0 || strcmp(class,"CDS")==0){	/* Save the exon or CDS */
				ecread();
				goto nextr;
			}
