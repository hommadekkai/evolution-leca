...
char fnamer[122]="/Users/khomma/work/ensembl/spomb/flongexgenes4";
char fnamel[100]="/Users/khomma/work/uniprot/uniprot_sprot_211117.dat";
char fnamew[122]="/Users/khomma/work/ensembl/spomb/flongingo3";
...
nextr:          strncpy(lstr,empty,5001);
	...
	/* Read in the GO # annotations to human proteins */
	strncpy(lstr,empty,5001);	oksp = 0;
        while((fgets(lstr,5000,fl))!=NULL){
		if(strncmp(lstr,"ID",2)==0 && strstr(lstr,"_SCHPO")!=NULL){
			oksp = 1;
			++counter;
			if(counter%2000==0)	printf("Now processing protein %d...\n",counter);
		}
		else if(oksp==1 && strncmp(lstr,"GN",2)==0 && strstr(lstr,"ORFNames=")!=NULL && binnumb<0){
