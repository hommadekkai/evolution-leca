/* mlongingo3.c   This program deteremines the GO #s that preferentially occur in proteins with long internal exons  220211
	basic.c -> hexonlensubloc1.c: A new program [R2107 I].	
	-> hexonlensubloc1M.c: Modified to process a different set of human proteins [R2107 I].
	-> hlongingo1.c: Modified to identify GO #s [R2201 I].
	-> flongingo1.c: Modified to process fission yeast instead of human data [R2201 II].
	-> flongingo1N.c: Modified to include single-exon and 2-exon genes in the comparison group [R2201 II].
	-> mlongingo1N.c: Modified to process mouse instead of fission yeast data [R2201 V].
	-> mlongingo3.c: Modified to identify GO IDs that preferentially occurr in genes with max exon len>=721 bp [R2202 VIII]. */
...
nextr:          strncpy(lstr,empty,5001);
	...
	strncpy(lstr,empty,5001);	oksp = 0;
		...
		if(strncmp(lstr,"ID",2)==0 && strstr(lstr,"_MOUSE")!=NULL){
			oksp = 1;
			++counter;
			if(counter%2000==0)	printf("Now processing protein %d...\n",counter);
		}
		else if(oksp==1 && strstr(lstr,"DR   Ensembl;")!=NULL && binnumb<0){
