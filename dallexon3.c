...
void ecread(char rstr[1001]){	/* Parse the exon line to get the sense, start, end, and chromosome # */
	...
	for(x=0; x<20; x++){
		...
		else if(rstr[x]=='_')	break;	/* #### as in mitochondrion_genome */
...
{
	char rstr[1001];
        char fnamer[122]="/Users/khomma/work/ensembl/dmel/Drosophila_melanogaster.BDGP6.22.97.chr.gff3";
        char fnamew[122]="/Users/khomma/work/ensembl/dmel/dallexon3";		/* #### */
	...
	fputs(">Chromosome\tGene\tTranscript\tLength\tExStart\tExEnd\tSense\tExon#\tTotEx#\tStart\tEnd\n",fw);  /* #### */
		/* Class: F: 5'-term, T: 3'-term, S: single exon */
	...
	for(x=0; x<40000; x++){
		...
		else if(strstr(rstr,"mRNA")!=NULL && strstr(rstr,"FlyBase")!=NULL){
			/* Possibly a new mRNA, i.e., a new set of exons and CDS must be initiated */
                        /* Verify */
                        for(x=0; x<20; x++)     if(isspace(rstr[x])!=0) break;          /* Chromosome # */
                        offset = x + 1;
                        for(x=0; x<50; x++)     if(isspace(rstr[x+offset])==0)  break;
                        offset = x + offset;
                        for(x=0; x<50; x++)     if(isspace(rstr[x+offset])!=0) break; /* The end of FlyBase */
                        offset = x + offset;
                        for(x=0; x<50; x++)     if(isspace(rstr[x+offset])==0)  break;
                        offset = x + offset;
                        strncpy(tempstr,empty,21);      /* Supposed to be mRNA */
                        for(x=0; x<20; x++){
                                if(isspace(rstr[x+offset])!=0)  break;
                                tempstr[x] = rstr[x+offset];
                        }
                        if(strcmp(tempstr,"mRNA")!=0)   goto nextr;     /* Was a false hit */
			if(gflag==1 && exmax>=0 && cdsmax>=0){  /* Output the last variant */
                                ++hit;
                                if(exmax>emax)  emax = exmax;
				calclen();      /* Calculate the length of the CDS */
				/* #### DO NOT eliminate exons that have no CDS region */
                        ...
                        /* Read in the transcript name (e.g., "FBtr0330654" */
				...
				for(y=0; y<6; y++)	exon[x][y]=0;		/* &&& */
		...
		else if(strstr(rstr,"gene")!=NULL && strstr(rstr,"FlyBase")!=NULL){
                        if(strstr(rstr,"ID=gene:")==NULL)       goto nextr;
                        ps = strstr(rstr,"FlyBase");
                        for(x=0; x<50; x++){
                                ++ps;
                        ...
                        strncpy(tempstr,empty,21);
                        for(x=0; x<20; x++){    /* gene, pseudognene, etc. */
                                if(isspace(*ps)!=0)     break;
                                tempstr[x] = *ps;
                                ++ps;
                        }
                        if(strcmp(tempstr,"gene")!=0)   goto nextr;     /* Not a gene */
                        ++counter;
				...
				/* #### DO NOT eliminate exons that have no CDS region */
                        ...
                        /* Read in the gene name (e.g., "FBgn0031208") */
		...
		else if(gflag==1 && (strstr(rstr,"exon")!=NULL || strstr(rstr,"CDS")!=NULL)){
                        ecread(rstr);
                }
