/* mscopconstex3.c    This program compares the obs freq. of SCOP-constrained internal exons with the expected freq.          220510
	 ...
	 -> hrexonscopavg3.c: Revised to use a list of internal exons by a corrected definition[R2204 I].
	 -> hrexonscopavg3A.c: Modified to neglect proteins without SCOP assingments [R2204 I]. (DEBUGGED)
	 -> hscopconstex1A.c: Major revision to analyze SCOP-constrained exons [R2204 I].
	 -> hscopconstex1.c: Modified not to neglect proteins without SCOP domains (###) [R2204 I].
	 -> hscopconstex1M.c: Modified so that SCOP domains partially encoded by internal exons are partially analysed "#+#" [R2204 I].
	 -> hscopconstex3.c: Simplified the algorithm to calculate probablitiy in each gene (#++#) [R2204 I].
	 -> mscopconstex3.c: Revised to process mouse instead of human data [R2204 VII]. */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <ctype.h>
...
char fnamel[100]="/Users/khomma/work/ensembl/mouse/malluniqexon3M";    /*  Non-redundant list of exons, with int CDS length */
char fnamew[122]="/Users/khomma/work/ensembl/mouse/mscopconstex3";     /* #+# */
                                        ...
                                        for(x=0; x<=gmax; x++){
                                                if(strstr(lstr,gname[x])!=NULL){        /* Possible hit. Check the total length */
                                                        ...
                                                        if(length==w){  /* HIT! */
                                                                ...
                                                                oldintcds = intcds;     /* The first transcript */
                                        ...
                                        for(x=0; x<=gmax; x++){
                                                if(strstr(lstr,gname[x])!=NULL){        /* Possible hit. Check the total length */
                                                        ...
                                                        if(length==w){  /* HIT! */
                                                                ...
                                                                exon[exmax][0] = exonparse(10);
                                                                exon[exmax][1] = exonparse(11);
                                                                t = exonparse(8);               /* #### Exon number */
                                                                u = exonparse(9);               /* #### Total exon number */
                                                                if(u==1)        exon[exmax][2] = 0;     /* #### Single exon */
                                                                else if(t==1)   exon[exmax][2] = 1;     /* #### 5'-term exon */
                                                                ...
                                                                intend = exonparse(13);         /* #"# */
                                                                intcds = intend - intstart +1;  /* #"# */
                                                                if(intcds!=oldintcds)      break;  /* The end of the transcript */
